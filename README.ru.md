...
<p align="center"><img src="docs/assets/pantheon/hero.png" alt="Арес в белом мраморе у классической колонны: гребенчатый шлем, копье острием вниз в покое и круглый щит, на котором одна линия прошла путь от красного к зеленому. Чистые красные нити уносят доказанную находку наружу; приглушенно-золотые нити он держит близко в руке." width="100%"></p>
...
Эта регрессия потом живет в твоем гейте навсегда. Это полный нативный Деймос, фазы A-D, с нулем внешних зависимостей. Живую цель он бьет только на 127.0.0.1, под своей песочницей, и наружу с машины не выходит.
...
Работает: полный нативный Деймос A-D, ядро анти-театра, doctor вендоренного ядра и сквозной приемочный сценарий проходят гейт. Впереди: живой seat-adapter при сборке роя Olympuz и переносимая песочница, чтобы живая цель поднималась не только на macOS.
...
`docs/MASTER-PLAN.md` секвенсит фазы Деймоса A-D по зависимости. `docs/ONBOARDING.md` проводит первую установку по шагам. `AGENTS.md` держит доктрину и инварианты. `SECURITY.md` держит модель безопасности. `tests/CORPUS.md` называет каждую пробу.
